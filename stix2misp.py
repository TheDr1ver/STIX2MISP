...
#
# USAGE:
#   python3 stix2misp.py -i <file_name>
#
    ...
    if "FileObjectType" in str(object_type):
        ...
        for hash in properties.hashes:
            ...
            if str(hash.type_) == "MD5":
                ...
                misp.add_hashes(event, filename=str(properties.file_name), md5=str(hash), comment=comment,
                                to_ids=args.ids)

            elif str(hash.type_) == "SHA1":
                # Add the hash by itself
                # misp.add_hashes(event, sha1=str(hash))
                misp.add_hashes(event, filename=str(properties.file_name), sha1=str(hash), comment=comment,
                                to_ids=args.ids)

            elif str(hash.type_) == "SHA256":
                # Add the hash by itself
                # misp.add_hashes(event, sha256=str(hash))
                misp.add_hashes(event, filename=str(properties.file_name), sha256=str(hash), comment=comment,
                                to_ids=args.ids)

            elif str(hash.type_) == "SSDEEP":
                # Add the hash by itself
                # misp.add_hashes(event, ssdeep=str(hash))
                misp.add_hashes(event, filename=str(properties.file_name), ssdeep=str(hash), comment=comment,
                                to_ids=args.ids)
...
def processSTIX(pkg, args, misp_url, misp_key, ssl=True):
    ...
    if misp_title == "None":
        try:
            misp_title = str(pkg.stix_header.information_source.description) + " | " + str(
...
if __name__ == "__main__":
    ...
    misp_url = 'https://192.168.2.133/users/login'
    misp_key = ''

    # Input Parser

    parser = argparse.ArgumentParser(
        description="EXAMPLE: stix2misp.py -i stix_file.xml -T \"custom tag, tag2\" -F -m \"^[A-Z0-9-]*$\"")
    parser.add_argument("-i", "--input", type=str, required=True, help="STIX file to upload to MISP")
    parser.add_argument("-d", "--distrib", type=int,
                        help="The distribution setting used for the attributes and for the newly created event, if relevant. [0-3]. DEFAULT: 0")
    ...
    parser.add_argument("-a", "--analysis", type=int,
                        help="The analysis level of the newly created event, if applicatble. [0-2] DEFAULT: 2")
    parser.add_argument("-t", "--threat", type=int,
                        help="The threat level ID of the newly created event, if applicatble. [1-4] DEFAULT: 3")
    parser.add_argument("-T", "--tag", type=str, help="Add a comma-separated list of tags to add to the event")
    parser.add_argument("-F", "--forcetag", action='store_true',
                        help="Automatically adds the Title and ID of the STIX package as tags")
    parser.add_argument("-m", "--mask", type=str,
                        help="Add a Regex mask for the forcetag option to avoid unnecessarily long tagging (e.g. \"^[A-Z0-9-]*$\"). Must be run in conjunction with -F.")
